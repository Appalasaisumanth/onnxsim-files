...
def _parse_dram_stats(content: str) -> dict:
    ...
    bw = re.search(
        r"HBM2-CH_\d+: avg BW utilization (\d+)%\s+\((\d+) reads,\s+(\d+) writes\)", content)
    if bw:
...
def build_flat_summary_row(data: dict, log_file: str) -> dict:
    ...
    # Layer-type cycle breakdown — flattened as individual columns
...
def aggregate_mamba_runs(flat_rows: list) -> list:
    """
    Given a list of flat summary rows (one per log file), group runs that
    belong to the same model execution and produce one aggregated row per group.
    Rows for non-Mamba models (OPT etc.) pass through unchanged (group of 1).
    """
    ...
    for key, rows in groups.items():
        if len(rows) == 1:
            # Single run (OPT or single-step Mamba) — no aggregation needed
            row = dict(rows[0])
        ...
        if not ok_rows:
            # All steps crashed — keep first row as representative
            row = dict(rows[0])
        ...
        # ── SUM columns (things that accumulate across generation steps) ──
        sum_cols = [
        ...
        # ── MEAN columns (rates, utilisation — average across steps) ──────
        mean_cols = [
        ...
        # Recompute derived pct columns from summed values
        total_lc = sum(
            agg.get(c, 0) or 0 for c in [
                "layer_gemm_cycles", "layer_attention_cycles",
                "layer_ffn_fc1_cycles", "layer_ffn_fc2_cycles",
                "layer_mamba_in_proj_cycles", "layer_mamba_x_proj_cycles",
                "layer_mamba_dt_proj_cycles", "layer_mamba_out_proj_cycles",
                "layer_mamba_ssm_cycles",
        ...
        # Recompute total SRAM hit rates from summed hits/misses
        for sk in ["sram", "acc_sram"]:
